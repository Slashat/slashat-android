...
public class HighFiveGetUserAsyncTask extends AsyncTask<String, Void, User> {
    ...
    private User getUserFromJson(JSONObject jsonObject) throws JSONException, MalformedURLException {
        ...
        return new User(username,userId, highFivedBy, new URL(pictureUrl), new URL(qrcode), qrcode_id, highFivers);
    }

    private Collection<HighFiver> getHighFiversFromJson(JSONObject highfivers) throws JSONException, MalformedURLException {
        ArrayList<HighFiver> highFivers = new ArrayList<HighFiver>();

        Iterator keys = highfivers.keys();

        while(keys.hasNext()){
            String nextKey = (String) keys.next();
            JSONObject jsonObject = (JSONObject) highfivers.get(nextKey);
            String userName = jsonObject.getString("username");
            String pictureUrl = jsonObject.getString("picture");
